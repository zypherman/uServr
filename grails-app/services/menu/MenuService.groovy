...
class MenuService {
    ...
    ArrayList drinks = new ArrayList<DrinkDTO>(Arrays.asList(
            new DrinkDTO(name: 'Water', description: 'Tapped from a Glacier', size: '12 oz', pour: 'Glass', price: 0.00, cat: 'non-alcohol'),
            new DrinkDTO(name: 'Coke', description: 'Soda', size: '12 oz', pour: 'Glass', price: 1.00, cat: 'non-alcohol'),
            new DrinkDTO(name: 'Sprite', description: 'Soda', size: '12 oz', pour: 'Glass', price: 1.00, cat: 'non-alcohol'),
            new DrinkDTO(name: 'Mellow Yellow', description: 'Soda', size: '12 oz', pour: 'Glass', price: 1.00, cat: 'non-alcohol'),
            new DrinkDTO(name: 'Diet Coke', description: 'Soda', size: '12 oz', pour: 'Glass', price: 1.00, cat: 'non-alcohol'),
            new DrinkDTO(name: 'Lemonade', description: 'Freshly Squeezed Lemons', size: '12 oz', pour: 'Glass', price: 2.00, cat: 'non-alcohol'),
            new DrinkDTO(name: 'San Pellegrino', description: 'Sparkling Mineral Water', size: '16.9 oz', pour: 'Bottle', price: 10.00, imgSrc: 'Sanpellegrino.jpg', cat: 'non-alcohol'),
    ...
    def findItem(String name) {
        for (def drink : drinks) {
            if ((drink.name as String).equalsIgnoreCase(name)) {
                return drink
            }
        }
        for (def foods : food) {
            if ((foods.name as String).equalsIgnoreCase(name)) {
                return foods
            }
        }
    }

    def removeMenuItem(JSONObject item) {
        for (def drink : drinks) {
        ...
        for (def foods : food) {
            if (foods.name == item.name) {
                return food.remove(foods)
            }
        }
    }
